...
- 기본 학습 인터페이스는 repo skill `$next`, 일반 대화, `$done`이다. `$next`는 새 수업 시작에만 사용한다. 활성 수업의 이론·질문·워크북·코딩 실습은 일반 대화로 진행하고, `$done`은 해당 수업을 검증하고 종료한다. literal `/next` 또는 `/done`이 일반 메시지로 전달되는 surface에서는 각각의 호환 별칭으로 처리한다.
- 다듬은 사용자 메시지가 정확히 `$next`이면 `PROGRESS.md`를 읽는다. `Active lesson`이 `none`이 아니면 상태·답안을 변경하지 않는다. 이 경우 활성 수업을 일반 대화로 이어가도록 안내한다. 활성 수업이 없고 `not_started` 행이 있으면 가장 낮은 번호를 `in_progress`로 표시하고 활성 수업으로 설정한 뒤 목표와 어휘부터 시작한다. `not_started` 행이 없으면 전체 연습 문제 모음을 검증하고 최종 복습 지도를 제공하되 `PROGRESS.md`를 변경하지 않는다.
- 새 수업을 활성화한 뒤에는 `$next`를 다시 요구하지 않는다. `lessons/`의 이론을 일반 대화로 한 번에 한 개념씩 설명하고 질의응답을 마친 뒤, 워크북과 코딩 실습을 순서대로 진행한다. 새 세션에서 `in_progress` 수업을 재개하면 `$next` 없이 일반 대화로 목표와 어휘 및 이론의 첫 개념부터 다시 시작한다.
- 일반 대화로 현재 워크북의 학습자 답안을 확인한다. 개념적으로 완성된 답안은 의미를 보존하면서 `LEARNER_TASK` 표시를 제거하고 오탈자·공백·문장부호·Markdown 같은 기계적 표기만 정리한다. 아직 시도하지 않았거나 개념적으로 불완전한 답안은 대신 완성하지 않고 실행 가능한 피드백 하나만 제공한다.
