...
- `$next`로 활성 수업을 재개할 때 현재 워크북의 학습자 답안을 확인한다. 개념적으로 완성된 답안은 의미를 보존하면서 `LEARNER_TASK` 표시를 제거하고 오탈자·공백·문장부호·Markdown 같은 기계적 표기만 정리한다. 아직 시도하지 않았거나 개념적으로 불완전한 답안은 대신 완성하지 않고 실행 가능한 피드백 하나만 제공한다.
